...
def calcular_Q_vc_max(t):  ##para efeitos de lidar com as incertezas, utiliza-se a média dos valores de Qvc max e min
    return (-1.19333e-7 * t**7 - 4.90754e-5 * t**6 + 3.733e-3 * t**5 -
            0.09621 * t**4 + 1.03965 * t**3 - 3.8645 * t**2 -
            1.0124 * t + 75.393 + 1.19333e-7 * t**7 - 6.54846e-5 * t**6 + 4.1432e-3 * t**5
    - 0.100585 * t**4 + 1.05575 * t**3 - 3.85966 * t**2 - 1.32657 * t + 75.393)/(2*3600)
...
def simulador_hidraulico(x):
    ...
    for t in range(24):
        ...
        if estado_bomba[t] == 1:
            ...
            densidade = 1000
            g = 9.81
            potencia = (Q_pump * densidade * g * h_pump) / (eficiencia * 1000)
            energia_por_hora.append(potencia)
        ...
        # Penalização por violação dos limites [2, 7] metros
        if nova_altura < altura_min or nova_altura > altura_max:
            custo_total += 5
            custo_por_hora[-1] += 5  # penaliza a hora corrente
    ...
    return {
        'fObj': custo_total,
        'g1': resultados_h_atual[1:],  # ignora h inicial
        'Q_r': resultados_Q_r,
        ...
        'energia_por_hora': energia_por_hora 
    }
...
def fun_constr_3(x):
    res = simulador_hidraulico(x)
    altura_final = res['h'][-1]
    return altura_final - h0
...
c1 = NonlinearConstraint(fun_constr_1, 1.0, 8.0, jac='2-point', hess=BFGS())  ##altura entre 1 e 8 metros, ao invés de 2 e 7 metros. Simula a incerteza da operação do reservatório devido a variação de caudais
c2 = NonlinearConstraint(fun_constr_2, -np.inf, 0.0, jac='2-point', hess=BFGS())
c3 = NonlinearConstraint(fun_constr_3, 0.0, np.inf, jac='2-point', hess=BFGS())


x0 = [4, 15] + [5, 7]  # palpite inicial
...
print("Hora | Bomba | Q_r (m³/s) | Q_vc_max (m³/s) | Q_pump (m³/s) | altura_atual (m)")
print("-----|-------|------------|-----------------|---------------|------------------")
...
ax1.set_ylim(0, altura_max + 1+1)
ax1.set_xticks(horas)
ax1.grid(True, linestyle='--', alpha=0.6)

# Linhas de referência para alturas
ax1.axhline(1, color='r', linestyle=':', linewidth=2, label='Altura Mín Esperada(1m)')
ax1.axhline(8, color='g', linestyle=':', linewidth=2, label='Altura Máx Esperada(8m)')
